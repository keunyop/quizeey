...
-- Q9
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '9', 'You have been asked to design a NAT solution for your company''s VPC-based web application. Traffic from the privatesubnets varies throughout the day from 500 Mbps to spikes of 7 Gbps.What is the most cost-effective and scalable solution? ', 'Getting StartedLet''s try to see how we can create and configure an AWS NAT Gateway: Login to the AWS console, select VPC service and click on NAT Gateways as shown below:Managed NAT gateway - dashboard Provide the necessary details, like subnet and Elastic IP, and create the NAT Gateway. You need the select the subnet which you want to be private subnet and your Elastic IP so that it can communicate to Internet.NAT Gateway - create Once created you will see this:NAT Gateway - success Once the NAT Gateway is created you can edit your routing table to send traffic destined for the Internet toward the gateway. The gateway''s internal address will be chosen automatically, and will be in the same subnet as the gateway. Once the NAT Gateway is configured, you are all set. Your private subnet instances should now be able to communicate with the Internet without much management, monitoring, and configuration overhead. Sample NAT Gateway architecture:Migrating from an existing NAT instanceIf you are already using a NAT instance in your VPC setup, it''s time to migrate now, and I can tell you that it''s not tough. You only need to make sure that you create the NAT Gateway in the same subnet as your existing NAT instance. Then you need to edit the route table by replacing the existing NAT reference with the internal address of the new gateway. I told you this was very straightforward. You will need to ensure that you don''t have any critical tasks running at the time of migration, because changing a route from a NAT instance to the gateway can result in a dropped connection. This feature was only very recently introduced by AWS, so it''s definitely worth sharing. It can resolve lots of existing concerns. Do you have your own experience with this new feature? Why not share it with others.', '', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '9', '4', 'Create a NAT gateway in a public subnet; route all private subnet Internet Traffic through the NAT gateway', 'Y', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '20', '4', 'Amazon DynamoDB', 'Y', now(), now());
...
-- Q34
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '34', 'Which of the following statements are true about Amazon Route 53 resource records? Choose 2 answers ', 'https://aws.amazon.com/route53/faqs/ Amazon Route 53 offers ''Alias'' records (an Amazon Route 53-specific virtual record). Alias records are used to map resource record sets in your hosted zone to Amazon Elastic Load Balancing load balancers, Amazon CloudFront distributions, AWS Elastic Beanstalk environments, or Amazon S3 buckets that are configured as websites. Alias records work like a CNAME record in that you can map one DNS name (example.com) to another ''target'' DNS name (elb1234.elb.amazonaws.com). http://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resource-record-sets-choosing-aliasnon-alias.html A CNAME record can point to any DNS record hosted anywhere, including to the resource record set that Amazon Route 53 automatically creates when you create a policy record.', '', 'N', now(), now());
...
-- Q35
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '35', 'Which aspects of Amazon EC2 security are the responsibility of AWS? Choose 2 answers ', 'AWS is responsible for protecting the global infrastructure that runs all of the services offered in the AWS cloud. This infrastructure is comprised of the hardware, software, networking, and facilities that run AWS services. AWS provide several reports from third-party auditors who have verified their compliance with a variety of computer security standards and regulations AWS is responsible for the security configuration of its products that are considered managed services for e.g. RDS, DynamoDB For Managed Services, AWS will handle basic security tasks like guest operating system (OS) and database patching, firewall configuration, and disaster recovery. AWS Security Responsibilities', '', 'N', now(), now());
...
-- Q38
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '38', 'A client application requires operating system privileges on a relational database server. What is an appropriate configuration for a highly available database architecture? ', 'http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html', '', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '38', '4', 'Amazon EC2 instances in a replication configuration utilizing two different Availability Zones', 'Y', now(), now());
...
-- Q41
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '41', 'You are working with a customer who is using Chef configuration management in their data center. Which service is designed to let the customer leverage existing Chef recipes in AWS? ', '', 'http://aws.amazon.com/opsworks/', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '41', '4', 'AWS OpsWorks', 'Y', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '122', '2', 'The outbound network ACL needs to be modified to allow outbound traffic.', 'Y', now(), now());
...
-- Q142
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '142', 'You try to connect via SSH to a newly created Amazon EC2 instance and get one of the following error messages: "Network error: Connection timed out" or "Error connecting to [instance], reason: -> Connection timed out: connect," You have confirmed that the network and security group rules are configured correctly and the instance is passing status checks. What steps should you take to identify the source of the behavior? Choose 2 answers ', '', 'http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/TroubleshootingInstancesConnect ing.html', 'Y', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '142', '5', 'Verify that you are connecting with the appropriate user name for your AMI.', 'Y', now(), now());
...
-- Q175
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '175', 'What are characteristics of Amazon S3? Choose 2 answers ', '', 'http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-restrictingaccess-to-s3.', 'N', now(), now());
...
-- Q176
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '176', 'Which of the following are true regarding AWS CloudTrail? Choose 3 answers ', '', 'http://aws.amazon.com/cloudtrail/faqs/', 'Y', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '198', '3', 'Create an A record aliased to the load balancer DNS name', 'Y', now(), now());
...
-- Q227
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '227', 'You have a video transcoding application running on Amazon EC2. Each instance polls aqueue to find out which video should be transcoded, and then runs a transcoding process. If this process is interrupted, the video will be transcoded by another instance based on the queuing system. You have a large backlog of videos which need to be transcoded and would like to reduce this backlog by adding more instances. You will need these instances only until the backlog is reduced. Which type of Amazon EC2 instances should you use to reduce the backlog in the most cost efficient way? ', '', 'http://aws.amazon.com/ec2/purchasing-options/spot-instances/', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '227', '4', 'Spot instances', 'Y', now(), now());

-- Q228
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '228', 'Which features can be used to restrict access to data in S3? Choose 2 answers ', '', 'http://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-restrictingaccess-to-s3.', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '236', '4', 'CodeDeploy', 'Y', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '247', '4', 'Enable both CloudTrail and VPC Flow Logs for the production VP', 'Y', now(), now());
...
-- Q301
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '301', 'You are running a mobile media application and are considering API Gateway for the client entry point. What benefits would this provide? Choose 2 answers ', 'API Logging, Caching, Throttling, Bursting and MonitoringInstead of invoking the backend API for every client call, developers can configure caching which will improve performance. Cache settings allow developers to control the way the cache key is built and the time-to-live (TTL) of the data stored for each method. The management API can be invoked to invalidate the cache. The pricing for this feature is based on the size of the cache. Developers need to protect their backend APIs from deliberate misuse. They may also want to restrict the rate at which the API is invoked. Through throttling, it is possible to set a rate limit of API requests to avoid sudden spikes. API Gateway can also be configured to handle bursts of API calls for specific scenarios. Clients can include automatic retry logic when they receive a 429 HTTP response as a result of throttling. With the right configuration of cache and throttling, developers can increase the performance while securing the backend API. Since API Gateway is a crucial element of an application, it needs to be monitored for uptime. For custom gateways, administrators need to configure native monitoring combined with agents. Amazon API Gateway integrates with CloudWatch, the one-stop monitoring service for all AWS services. By configuring an IAM role that has write access to CloudWatch logs, API Gateway can be monitored for a variety of metrics. image00Configuring Caching, Logging, Monitoring and Throttling', '', 'N', now(), now());
...
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '317', '4', 'Web Identity Federation', 'Y', now(), now());
...
-- Q335
insert into question (test_id, ver_nbr, quest_nbr, quest_txt, explanation, reference, multi_answer_yn, created_date, modified_date) values ('143', '214', '335', 'You are configuring your company''s application to use Auto Scaling and need to move user state information. Which of the following AWS services provides a shared data store with durability and low latency? ', '', 'https://d36cz9buwru1tt.cloudfront.net/AWS_Overview.pdf (page 13, aws storage gateway)', 'N', now(), now());
insert into example (test_id, ver_nbr, quest_nbr, exmp_nbr, exmp_txt, answer_yn, created_date, modified_date) values ('143', '214', '335', '1', 'Amazon DynamoDB', 'Y', now(), now());
